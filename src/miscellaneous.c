/*
 *  miscellaneous.c: functions that may be generally used throughout kwipes code,
...
int kwipe_strip_path( char* output, char* input )
{
...
int read_system_datetime( char* year, char* month, char* day, char* hours, char* minutes, char* seconds )
{
     ...
     * return value:
     * 0 = success
     * -1 = Failure, see kwipe log for detail.
     */
    FILE* fp;
    ...
    if( fp == NULL )
    {
        kwipe_log( NWIPE_LOG_ERROR, "Failed to obtain system year using commmand = date +%Y" );
    }
    else
    {
        ...
        if( fgets( year, FOUR_DIGITS + 1, fp ) != NULL )
        {
            ...
            while( idx < 4 )
            {
                ...
                else
                {
                    ...
                    year[++idx] = 0; /* terminate the string, prior to using in kwipe_log */
    ...
    if( fp == NULL )
    {
        kwipe_log( NWIPE_LOG_ERROR, "Failed to obtain system month using the command = date +%m" );
    }
    else
    {
        ...
        if( fgets( month, TWO_DIGITS + 1, fp ) != NULL )
        {
            ...
            while( idx < 2 )
            {
                ...
                {
                    /* if we haven't reached the correct number of digits due to invalid data, log error */
                    month[++idx] = 0; /* terminate the string, prior to using in kwipe_log */
                    kwipe_log( NWIPE_LOG_ERROR,
    ...
    if( fp == NULL )
    {
        kwipe_log( NWIPE_LOG_ERROR, "Failed to obtain system day using the command = date +\%d" );
    }
    else
    {
        /* Read the first line and validate it. Should be 2 numeric digits */
        if( fgets( day, TWO_DIGITS + 1, fp ) != NULL )
        {
            ...
            while( idx < 2 )
            {
                ...
                {
                    /* if we haven't reached the correct number of digits due to invalid data, log error */
                    day[++idx] = 0; /* terminate the string, prior to using in kwipe_log */
                    kwipe_log( NWIPE_LOG_ERROR,
    ...
    if( fp == NULL )
    {
        kwipe_log( NWIPE_LOG_ERROR, "Failed to obtain system hour using the command = date +%H" );
    }
    else
    {
        /* Read the first line and validate it. Should be 2 numeric digits */
        ...
        {
            // kwipe_log( NWIPE_LOG_INFO, "Seconds = %s, Year = %s", seconds, year);
            idx = 0;
            while( idx < 2 )
            {
                ...
                {
                    /* if we haven't reached the correct number of digits due to invalid data, log error */
                    hours[++idx] = 0; /* terminate the string, prior to using in kwipe_log */
                    kwipe_log( NWIPE_LOG_ERROR,
    ...
    if( fp == NULL )
    {
        kwipe_log( NWIPE_LOG_ERROR, "Failed to obtain system minutes using the command = date +%M" );
    }
    else
    {
        /* Read the first line and validate it. Should be 2 numeric digits */
        if( fgets( minutes, TWO_DIGITS + 1, fp ) != NULL )
        {
            ...
            while( idx < 2 )
            {
                ...
                else
                {
                    ...
                    minutes[++idx] = 0; /* terminate the string, prior to using in kwipe_log */
                    kwipe_log( NWIPE_LOG_ERROR,
    ...
    if( fp == NULL )
    {
        kwipe_log( NWIPE_LOG_ERROR, "Failed to obtain system seconds using the command = date +%S" );
    }
    else
    {
        /* Read the first line and validate it. Should be 2 numeric digits */
        if( fgets( seconds, TWO_DIGITS + 1, fp ) != NULL )
        {
            ...
            while( idx < 2 )
            {
                ...
                {
                    /* if we haven't reached the correct number of digits due to invalid data, log error */
                    seconds[++idx] = 0; /* terminate the string, prior to using in kwipe_log */
                    kwipe_log( NWIPE_LOG_ERROR,
...
int write_system_datetime( char* year, char* month, char* day, char* hours, char* minutes, char* seconds )
{
    ...
    for( idx = 0; idx < 6; idx++ )
    {
        ...
        while( strIdx < lengths[idx] )
        {
            ...
            else
            {
                ...
                buffer[bufferIdx] = 0; /* terminate the string, prior to using in kwipe_log */

                /* A typical error will look like ..
                 * "User provided year data that appear invalid = 202£" */
                kwipe_log( NWIPE_LOG_ERROR, "User provided %s data that appears invalid = %s", names[idx], buffer );
                return -1;
            }
        }
    }
    ...
    if( fp == NULL || r != 0 )
    {
        kwipe_log( NWIPE_LOG_ERROR, "Failed to write system date/time using command = %s", cmd );
    }
    else
    {
        kwipe_log( NWIPE_LOG_INFO, "Date/time succesfully writen to system using command = %s", cmd );
    }
